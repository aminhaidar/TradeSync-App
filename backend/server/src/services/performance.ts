...
export class PerformanceService {
  ...
  async getPerformanceData(timeRange: '7d' | '30d' | '90d'): Promise<PerformanceData[]> {
    try {
      ...
      const response = await axios.get<AlpacaPortfolioHistory>(`${config.alpaca.trading.url}/v2/account/portfolio/history`, {
        headers: {
...
// Create the service instance with proper typing
declare global {
  var io: Server;
}

const ioInstance: Server = new Server();
export const performanceService = new PerformanceService(ioInstance); 